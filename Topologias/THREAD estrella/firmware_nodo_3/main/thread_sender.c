...
static esp_err_t thread_stack_start_common(void)
{
    ...
    /* Topologia estrella: este firmware debe quedar como End Device/Child.
     * No debe convertirse en Router ni reenviar paquetes.
     */
    ot_err = otThreadSetRouterEligible(ins, false);
    if (ot_err != OT_ERROR_NONE) {
        ESP_LOGW(OT_TAG, "otThreadSetRouterEligible(false) fallo: %d", ot_err);
    }
#endif
...
#if NODE_DEBUG_LOGS
                ESP_LOGI(OT_TAG, "Thread attached como Child/SED role=%d", role);
#endif
                return ESP_OK;
            }
#endif
