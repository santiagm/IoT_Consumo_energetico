...
static const char *TAG = "NODO_N2_INSTALL_CODE";
...
static void print_ieee802154_mac(void)
{
    ...
    if (err == ESP_OK) {
        ESP_LOGI("NODE_IEEE_N2",
                 "IEEE802154 real del nodo N2 para config.h gateway = {0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X}",
                 ieee[0], ieee[1], ieee[2], ieee[3],
                 ieee[4], ieee[5], ieee[6], ieee[7]);

        ESP_LOGI("NODE_IEEE_N2",
                 "IEEE802154 string N2 = %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X",
                 ieee[0], ieee[1], ieee[2], ieee[3],
                 ieee[4], ieee[5], ieee[6], ieee[7]);
    } else {
        ESP_LOGE("NODE_IEEE_N2",
...
void app_main(void)
{
    ...
    ESP_LOGW(TAG, "Iniciando firmware_nodo2_zigbee_install_code N2");
    ...
    while (!zigbee_sender_is_joined()) {
        ...
        if (elapsed_join_ms >= join_timeout_ms) {
            ESP_LOGW(TAG,
                     "Timeout esperando union Zigbee N2 (%u ms). Entrando en deep sleep para reintentar luego.",
        ...
        if (first_join_wait_log || ((now_tick - last_join_log_tick) >= join_log_period_ticks)) {
            ESP_LOGW(TAG, "Esperando union Zigbee N2...");
...
#if NODE_DEBUG_LOGS
    ESP_LOGW(TAG, "Nodo N2 Zigbee unido, enviando paquete");
#endif
    ...
    ESP_LOGI(TAG,
             "N2 batch=%u packet_id=%" PRIu32 " activo_ms=%lld [synth_ms=%lld crypto_ms=%lld zigbee_join_ms=%lld zigbee_send_ms=%lld post_tx_ms=%u]",
             (unsigned)packet.batch_id,
             packet.packet_id,
             t_total_ms,
             t_synth_ms,
