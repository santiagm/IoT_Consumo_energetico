                                            ...
                                            const char *install_code_hex)
{
    ...
    /*
    * IMPORTANTE:
    * esp_read_mac(ESP_MAC_IEEE802154) imprime el IEEE en orden visual:
    * 98:A3:16:FF:FE:96:8A:50
    *
    * Pero ezb_secur_ic_add() suele esperar el EUI64 en orden interno/little-endian.
    * Por eso se invierte aquí SOLO para registrar en el Trust Center.
    *
    * En config.h se mantiene el IEEE tal como lo imprime cada nodo.
    */
    for (int i = 0; i < 8; i++) {
        ext_addr.u8[i] = ieee_addr[7 - i];
    }

    log_ieee(ieee_addr);

    ESP_LOGI(TAG,
            "IEEE usado internamente para Trust Center=%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X",
            ext_addr.u8[0],
            ext_addr.u8[1],
            ext_addr.u8[2],
            ext_addr.u8[3],
