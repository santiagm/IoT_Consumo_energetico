...
/*
 * UUIDs privados del escenario 3 BLE seguro.
...
static uint8_t s_gatt_payload[5 + sizeof(sensor_packet_t) + AES_GCM_TAG_LEN];
...
static void start_advertising(void)
{
     ...
     * Solo anuncia la identidad del nodo N1/N2/N3 para que el BLE_gateway
     * conecte, cifre el enlace y lea la caracteristica GATT segura.
     */
    const size_t name_len = strlen(BLE_NODE_DEVICE_NAME);
    if ((name_len + 5U) > 31U) {
        ESP_LOGE(TAG, "Nombre BLE demasiado largo para advertising primario: %u", (unsigned)name_len);
        return;
    }
    ...
    adv_data[adv_idx++] = 0x01; /* Flags */
    adv_data[adv_idx++] = (BLE_HS_ADV_F_DISC_GEN | BLE_HS_ADV_F_BREDR_UNSUP);
    adv_data[adv_idx++] = (uint8_t)(name_len + 1U);
    adv_data[adv_idx++] = 0x09; /* Complete Local Name */
    memcpy(&adv_data[adv_idx], BLE_NODE_DEVICE_NAME, name_len);
    adv_idx += name_len;

    rc = ble_gap_adv_set_data(adv_data, (int)adv_idx);
    if (rc != 0) {
    ...
    if (rc != 0) {
        ...
        ESP_LOGI(TAG, "Advertising conectable iniciado (%s); telemetria via GATT seguro", BLE_NODE_DEVICE_NAME);
    }
}
                                      ...
                                      const uint8_t *auth_tag)
{
    ...
    s_gatt_payload[idx++] = (uint8_t)NODE_CRYPTO_ID;
