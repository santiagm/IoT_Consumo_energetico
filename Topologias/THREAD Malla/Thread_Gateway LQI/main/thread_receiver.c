                                        ...
                                        int16_t *neighbor_lqi)
{
    ...
    while (otThreadGetNextNeighborInfo(ins, &iterator, &nbr) == OT_ERROR_NONE) {
        ...
        /*
         * Si el peer del paquete no coincide con un vecino directo, usamos
         * el vecino mas reciente como respaldo. Esto es util en malla porque
         * el Gateway puede ver solo el ultimo salto directo.
         */
    ...
    if (neighbor_lqi != NULL) {
        /*
         * OpenThread LinkQualityIn usa escala pequena: 0..3.
         * 0 = mala/no usable; 3 = buena.
         */
...
static void udp_receive_callback(void *context, otMessage *message, const otMessageInfo *message_info)
{
    ...
    /*
     * Métricas desde tabla de vecinos OpenThread.
     * LQI = mLinkQualityIn en escala 0..3.
     * RSSI = mLastRssi o mAverageRssi del vecino usado.
     *
     * En malla, si no hay coincidencia exacta con el RLOC16 del peer,
     * se usa el vecino mas reciente como respaldo, que normalmente representa
     * el ultimo salto directo visible por el Gateway.
     */
    ...
    if (get_thread_neighbor_metrics(message_info, &neighbor_rssi, &neighbor_lqi)) {
        ...
        /*
         * Mantener LQI Thread en escala propia 0..3.
         * No usar otThreadLinkInfo.mLqi porque en esta plataforma entrega
         * valores crudos tipo 6..11 que no son la metrica de vecino.
         */
        ev.lqi = neighbor_lqi;

        ESP_LOGI(OT_TAG,
                 "DEBUG_THREAD_LINK_USED: RSSI=%d dBm, LQI_THREAD_0_3=%d",
                 ev.rssi,
                 ev.lqi);
    } else {
        /*
         * Respaldo solo para RSSI. El LQI queda -1 si no hay vecino,
         * para evitar mezclarlo con el LQI crudo 6..11 del paquete.
         */
        ev.rssi = otMessageGetRss(message);
        ev.lqi = -1;

        ESP_LOGW(OT_TAG,
                 "No se pudo obtener neighbor metrics Thread. RSSI=%d, LQI=-1",
                 ev.rssi);
    }
