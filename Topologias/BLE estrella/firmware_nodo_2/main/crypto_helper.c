...
static void build_iv(uint16_t batch_id, uint8_t node_crypto_id, uint8_t iv[AES_GCM_IV_LEN])
{
    memset(iv, 0, AES_GCM_IV_LEN);
    iv[0] = (uint8_t)(batch_id & 0xFF);
    iv[1] = (uint8_t)((batch_id >> 8) & 0xFF);
    iv[2] = node_crypto_id; /* evita reutilizar nonce AES-GCM entre N1/N2/N3 */
}
                                 ...
                                 uint8_t *auth_tag)
{
    ...
    build_iv(batch_id, node_crypto_id, iv);
