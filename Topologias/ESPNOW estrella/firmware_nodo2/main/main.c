 ...
 * ============================================================
 *
 * ESP-NOW + Seguridad PMK/LMK + AES-128-GCM de aplicacion + datos sinteticos + Deep Sleep con RTC = 10 segundos.
 *
 * No usa:
 * - MQTT en el nodo
 ...
 *
 * Sí usa:
 * - Datos sinteticos con el mismo payload del Escenario 4
 * - ESP-NOW con cifrado PMK/LMK
 * - WiFi STA solo para habilitar ESP-NOW
 * - Deep sleep por timer RTC cada 10 segundos
 *
 * Objetivo:
 * Medir consumo del nodo con ESP-NOW + seguridad nativa + AES-GCM,
 * sin MPU6050 fisico y manteniendo los tiempos del Escenario 4.
 *
 * Medición:
 * - Escenario derivado del Escenario 4.
 * - No usa MPU6050 fisico.
 */
...
#if NODE_DEBUG_LOGS
static const char *TAG = "NODO_SENSOR_ESC3_SEC";
...
static esp_err_t build_synthetic_packet(sensor_packet_t *packet)
{
    ...
    for (uint8_t i = 0; i < STREAM_SAMPLE_COUNT; i++) {
        ...
        int16_t base = (int16_t)((packet->batch_id * 37U) + (i * 11U));

        packet->muestras[i].ax = (int16_t)(120 + base + (int16_t)(r & 0x003FU));
        packet->muestras[i].ay = (int16_t)(-80 + base + (int16_t)((r >> 6) & 0x003FU));
        packet->muestras[i].az = (int16_t)(16384 + (int16_t)((r >> 12) & 0x007FU));
        packet->muestras[i].gx = (int16_t)(5 + base + (int16_t)((r >> 19) & 0x001FU));
        packet->muestras[i].gy = (int16_t)(-3 + base + (int16_t)((r >> 24) & 0x001FU));
        packet->muestras[i].gz = (int16_t)(2 + base + (int16_t)((r >> 29) & 0x0007U));
    }
...
#if NODE_DEBUG_LOGS
    ESP_LOGI(TAG, "Entrando a deep sleep por 5 segundos");
    fflush(stdout);
...
void app_main(void)
{
    ...
    NODE_LOGI(TAG, "ESCENARIO 3 ESTRELLA: ESP-NOW + Seguridad + AES-GCM + datos sinteticos + deep sleep RTC. Nodo N2");
     ...
     */
    err = esp_wifi_set_max_tx_power(20);
    if (err != ESP_OK) {
    ...
    /*
     * Construir un paquete sintetico y enviar un solo paquete ESP-NOW.
     */
