...
static void packet_consumer_task(void *pvParameter)
{
    ...
    while (1) {
        ...
        if (s_mqtt_client == NULL || ((bits & MQTT_CONNECTED_BIT) == 0)) {
            ESP_LOGW(TAG, "MQTT no conectado; paquete BLE descartado batch=%u", packet.batch_id);
            continue;
        }
        ...
        ESP_LOGI(TAG, "Publicado ThingsBoard batch=%u msg_id=%d", packet.batch_id, msg_id);
#endif

        cJSON_free(json_string);
#else
        int written = uart_write_bytes(UART_BRIDGE_PORT,
                                       (const char *)&packet,
                                       sizeof(sensor_packet_t));
        if (written != sizeof(sensor_packet_t)) {
            ESP_LOGW(TAG, "UART envio incompleto batch=%u bytes=%d esperado=%u",
                     packet.batch_id, written, (unsigned)sizeof(sensor_packet_t));
        }
#if GATEWAY_DEBUG_LOGS
        ESP_LOGI(TAG,
                 "BLE OK -> UART batch=%u bytes=%u ax0=%d ay0=%d az0=%d gx0=%d gy0=%d gz0=%d",
                 packet.batch_id,
...
void app_main(void)
{
    ...
    s_packet_queue = xQueueCreate(GATEWAY_PACKET_QUEUE_LEN, sizeof(sensor_packet_t));
...
#else
    ESP_LOGI(TAG, "Gateway en modo BLE-only. WiFi/MQTT desactivado.");
#endif

    xTaskCreate(packet_consumer_task, "packet_consumer", 8192, NULL, 5, NULL);

    ESP_LOGI(TAG, "Iniciando Gateway BLE seguro: escaneo + conexion + pairing/bonding + GATT...");
