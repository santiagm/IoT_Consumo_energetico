...
static const char *TAG = "BLE_PLAIN_GATT";

#define BLE_NODE_DEVICE_NAME        "ESC4_BLE_NODE"
#define BLE_EVT_SYNCED              BIT0
#define BLE_EVT_CONNECTED           BIT1
#define BLE_EVT_PAYLOAD_READ        BIT2

/*
 * UUIDs privados conservados del escenario 4 BLE.
...
static uint8_t s_gatt_payload[4 + sizeof(sensor_packet_t)];
                               ...
                               void *arg)
{
    ...
    ESP_LOGI(TAG, "Payload en claro entregado por GATT. conn=%u bytes=%u",
...
static void start_advertising(void)
{
     ...
     * e inicie la conexion BLE. El payload real viaja por GATT en claro.
     *
    ...
    if (rc != 0) {
        ...
        ESP_LOGI(TAG, "Advertising minimo conectable iniciado; telemetria via GATT en claro");
    }
}
...
static void ble_app_on_sync(void)
{
    ...
    ESP_LOGI(TAG, "NimBLE sincronizado. BLE puro sin pairing/bonding/cifrado");
...
esp_err_t ble_broadcaster_init(void)
{
    ...
    xEventGroupClearBits(s_ble_events,
                         BLE_EVT_SYNCED | BLE_EVT_CONNECTED | BLE_EVT_PAYLOAD_READ);
    ...
    /* BLE puro: sin pairing, sin bonding y sin cifrado obligatorio del enlace. */
                                      ...
                                      size_t plain_len)
{
    ...
    if (plain_len != sizeof(sensor_packet_t)) {
        ESP_LOGE(TAG, "Tamano payload invalido: %u esperado=%u",
                 (unsigned)plain_len, (unsigned)sizeof(sensor_packet_t));
    ...
    s_gatt_payload[idx++] = (uint8_t)(plain_len & 0xFF);
    s_gatt_payload[idx++] = (uint8_t)((plain_len >> 8) & 0xFF);
    memcpy(&s_gatt_payload[idx], plain_payload, plain_len);
    idx += plain_len;
    s_gatt_payload_len = idx;

    xEventGroupClearBits(s_ble_events, BLE_EVT_CONNECTED | BLE_EVT_PAYLOAD_READ);

    ESP_LOGI(TAG, "Payload BLE puro preparado. batch_id=%u bytes=%u", batch_id, (unsigned)s_gatt_payload_len);
    ...
    if ((bits & BLE_EVT_PAYLOAD_READ) == 0) {
        ESP_LOGW(TAG, "Timeout esperando lectura GATT del Gateway");
        return ESP_ERR_TIMEOUT;
    }

    ESP_LOGI(TAG, "Envio BLE puro completado. batch_id=%u", batch_id);

    /*
     * Se conserva el mismo tiempo de gracia optimizado del Escenario 4.
     * No agrega un retardo nuevo: mantiene BLE_POST_READ_GRACE_MS ya existente.
     */
