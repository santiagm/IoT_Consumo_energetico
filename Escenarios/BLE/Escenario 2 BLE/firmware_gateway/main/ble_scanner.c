...
static const char *TAG = "BLE_PLAIN_CLIENT";
...
static void read_plain_telemetry(uint16_t conn_handle);
...
static bool parse_and_queue_packet(const uint8_t *data, size_t len)
{
    const size_t plain_len = sizeof(sensor_packet_t);
    const size_t expected_len = 4 + plain_len;
    ...
    uint16_t batch_id = data[0] | ((uint16_t)data[1] << 8);
    uint16_t payload_len = data[2] | ((uint16_t)data[3] << 8);

    if (payload_len != plain_len) {
        ESP_LOGW(TAG, "payload_len invalido: recibido=%u esperado=%u",
                 payload_len, (unsigned)plain_len);
        return false;
    }
    ...
    memcpy(&packet, &data[4], sizeof(packet));
    ...
    if (xQueueSend(s_packet_queue, &packet, 0) != pdPASS) {
        ESP_LOGW(TAG, "Cola llena, paquete BLE perdido");
        return false;
    }

    ESP_LOGI(TAG, "Paquete BLE puro recibido batch_id=%u", batch_id);
    return true;
}
                       ...
                       void *arg)
{
    ...
    uint8_t buffer[4 + sizeof(sensor_packet_t)];
    ...
    parse_and_queue_packet(buffer, om_len);
    ble_gap_terminate(conn_handle, BLE_ERR_REM_USER_CONN_TERM);
    return 0;
}
...
static void read_plain_telemetry(uint16_t conn_handle)
{
    ...
    ESP_LOGI(TAG, "Leyendo caracteristica BLE pura. val_handle=%u", s_chr_val_handle);
                       ...
                       void *arg)
{
    ...
    if (error->status == 0 && chr != NULL) {
        ...
        ESP_LOGI(TAG, "Caracteristica BLE pura encontrada. val_handle=%u", s_chr_val_handle);
        return 0;
    }
    ...
    if (error->status == BLE_HS_EDONE) {
        ...
        ESP_LOGI(TAG, "Descubrimiento de caracteristica completado; iniciando lectura GATT");
        read_plain_telemetry(conn_handle);
        return 0;
    }
...
static void discover_plain_telemetry(uint16_t conn_handle)
{
    s_svc_start_handle = 0;
    s_svc_end_handle = 0;
    s_chr_val_handle = 0;

    ESP_LOGI(TAG, "Descubriendo servicio GATT de telemetria BLE pura");
                           ...
                           void *arg)
{
    ...
    discover_plain_telemetry(conn_handle);
    return 0;
}
...
static void connect_to_node(const struct ble_gap_disc_desc *disc)
{
    ...
    ESP_LOGI(TAG, "Nodo encontrado; conectando para lectura GATT BLE pura");
...
static void ble_scanner_start(void)
{
    ...
    if (rc != 0 && rc != BLE_HS_EALREADY) {
        ...
        ESP_LOGI(TAG, "Escaneo BLE iniciado para nodo GATT puro. interval=0x%04X window=0x%04X",
...
{
    ESP_LOGI(TAG, "NimBLE sincronizado. Gateway central listo para BLE puro");
    ble_scanner_start();
}
...
esp_err_t ble_scanner_init(void)
{
    ...
    /* BLE puro: sin configuracion de seguridad nativa obligatoria. */
