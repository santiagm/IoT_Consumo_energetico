...
static const char *TAG = "GATEWAY_ESC2";
 ...
 * ============================================================
 *
 * Gateway para ESP-NOW puro sin PMK/LMK y sin AES-GCM.
 * El nodo envia sensor_packet_t plano con datos sinteticos.
 * Se mantiene la logica MQTT y el mismo formato JSON.
 */
                           ...
                           int len)
{
    ...
    if (len != sizeof(sensor_packet_t)) {
        ESP_LOGW(TAG,
                 "Paquete ESP-NOW plano invalido, tamano %d esperado %d",
                 len,
                 (int)sizeof(sensor_packet_t));
        return;
    }
    ...
    memcpy(&received_packet.packet, data, sizeof(received_packet.packet));
    ...
    ESP_LOGI(TAG,
             "ESP-NOW plano recibido de %s, batch=%u, node_ts=%" PRIu32 " len=%d",
             received_packet.src_mac,
             (unsigned)received_packet.packet.batch_id,
...
static esp_err_t espnow_add_plain_peer(const uint8_t *peer_mac)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error agregando/modificando peer ESP-NOW: %s", esp_err_to_name(err));
        return err;
    }

    ESP_LOGI(TAG,
             "Peer ESP-NOW listo: %02X:%02X:%02X:%02X:%02X:%02X",
...
static esp_err_t espnow_init(void)
{
    ...
    err = espnow_add_plain_peer(NODE_PEER_MAC);
    if (err != ESP_OK) {
        return err;
    }
    ...
    ESP_LOGI(TAG, "ESP-NOW plano listo en canal %d", ESPNOW_CHANNEL);
    return ESP_OK;
}
...
void app_main(void)
{
    ...
    if (espnow_err != ESP_OK) {
        ESP_LOGE(TAG, "Error inicializando ESP-NOW plano");
    }
