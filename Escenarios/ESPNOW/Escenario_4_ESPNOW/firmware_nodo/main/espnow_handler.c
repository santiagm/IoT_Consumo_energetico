...
static const char *TAG = "ESPNOW_SEC_NODE";
                           ...
                           esp_now_send_status_t status)
{
    ...
    ESP_LOGI(TAG,
             "TX cifrada a %02X:%02X:%02X:%02X:%02X:%02X -> %s",
...
static esp_err_t init_espnow_secure_peer(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "No se pudo configurar PMK: %s", esp_err_to_name(err));
        return err;
    }
    ...
    memcpy(peer_info.lmk, s_lmk, sizeof(peer_info.lmk));
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "No se pudo agregar/modificar peer cifrado: %s", esp_err_to_name(err));
        return err;
    }

    ESP_LOGI(TAG,
             "Peer gateway cifrado listo: %02X:%02X:%02X:%02X:%02X:%02X canal=%d",
...
esp_err_t espnow_handler_send(const uint8_t *data, size_t len)
{
    ...
    ESP_LOGI(TAG, "Enviando paquete ESP-NOW cifrado len=%u", (unsigned)len);
